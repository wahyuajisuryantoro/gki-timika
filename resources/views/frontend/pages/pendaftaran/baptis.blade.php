...
@section('content')
<div class="container">
    <h2 class="mb-4 text-primary">Pendaftaran Baptis</h2>
    <form id="baptisForm" action="{{ route('pendaftaran.storeBaptis') }}" method="POST" enctype="multipart/form-data" class="rd-form">
        @csrf
        <div class="form-wrap">
        ...
        <div class="form-wrap">
            ...
            <label class="form-label-outside" for="tanggal_pelaksanaan">Tanggal Pelaksanaan Baptis</label>
        </div>
        <div class="form-wrap">
            <input type="file" class="form-input" id="kartu_keluarga" name="kartu_keluarga" accept=".pdf,.jpg,.jpeg,.png" required>
            <label class="form-label-outside" for="kartu_keluarga">Kartu Keluarga (PDF, JPG, JPEG, PNG - Max 2MB)</label>
        </div>
        <div class="form-wrap">
        ...
        <button type="submit" class="btn btn-primary" id="submitBtn">
            <span class="btn-text">Daftar Baptis</span>
...
document.addEventListener('DOMContentLoaded', function() {
    const form = document.getElementById('baptisForm');
    const submitBtn = document.getElementById('submitBtn');
    const btnText = submitBtn.querySelector('.btn-text');
    const btnLoading = submitBtn.querySelector('.btn-loading');
    
    // Validasi Nomor Induk Jemaat
    ...
    tanggalLahir.addEventListener('change', function() {
        ...
        // Untuk baptis, biasanya tidak ada batasan umur minimal yang ketat
        // Tapi bisa memberikan info jika umur terlalu tinggi
        if (age > 80) {
            Swal.fire({
                icon: 'info',
                title: 'Informasi',
                text: 'Untuk pendaftaran baptis di usia ini, silakan hubungi admin gereja',
    ...
    function checkNomorIndukExists(nomorInduk) {
        ...
        fetch('{{ route("pendaftaran.storeBaptis") }}', {
            method: 'POST',
            body: formData,
            headers: {
    ...
    function validateForm() {
        ...
        // Validasi nomor induk (hanya required)
    ...
    function submitForm() {
        ...
        // Submit form menggunakan FormData untuk handle file upload
        ...
        .then(data => {
            if (data.success) {
                Swal.fire({
                    ...
                    text: 'Pendaftaran baptis Anda telah berhasil diajukan',
                    confirmButtonColor: '#3085d6',
                    confirmButtonText: 'OK'
                }).then(() => {
                    // Reset form
    ...
    // Handle session alerts dari Laravel
    @if(session('success'))
        Swal.fire({
            icon: 'success',
            title: 'Berhasil!',
